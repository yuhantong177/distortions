...
The rescale parameters can either be fixed (historical behaviour) or described as ranges so the script can try multiple scale combinations during the search.
...
When the microscope magnification or tilt makes the exact scale uncertain, declare ``start``/``end``/``step`` for one or both axes.  Example: ``"x": {"start": 0.85, "end": 0.95, "step": 0.01}`` explores the values ``0.85, 0.86, …, 0.94`` while keeping backward compatibility with configuration files that only provide a single float.
...
Both ``align.py`` and ``distord.py`` report a Dice similarity coefficient between the EBSD speckle and the segmented image (values range from ``0`` for no overlap to ``1`` for a perfect match).  Early iterations often show values around ``0.1–0.3`` when the scale/rotation are still off or when the segmentation is noisy; the absolute value is therefore less important than the trend.  Inspect the ``overlap.align.<id>.png`` and ``overlap.distord.<id>.png`` files to see where the residual mismatch comes from.

If the score stalls at a low value:

* Widen or refine the rotation/translation search ranges (and now the rescale ranges) in the alignment configuration.
* Increase the CMA-ES exploration by raising ``-num_sampling`` (e.g. ``-num_sampling 2000``) or reducing ``-mesh_step`` so more control points describe the distortion.
* Double-check the segmentation threshold – the Dice score is computed on the binarised images (``>=128``), therefore major intensity differences between modalities can artificially keep the score low.
