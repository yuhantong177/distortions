...
{
  "rescale": {
    "x": {"start": 0.8, "end": 1.1, "step": 0.05},
    "y": {"start": 0.8, "end": 1.0, "step": 0.05}
  },
  "grid_search": { ... },
  "search": {
    "mode": "random",   # or omit the block to keep the exhaustive grid
    "samples": 2000,      # number of random combinations to evaluate
    "seed": 123           # optional: makes runs reproducible
  }
}
```

The random mode still honours the discrete ``start``/``end``/``step`` values you provided; it simply samples a subset of the cartesian grid, dramatically reducing runtime when exploring wide ranges.
